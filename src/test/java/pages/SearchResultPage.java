...
public class SearchResultPage extends BasePage {
	...
	@FindBy(xpath = "//div[@id='rso']/div[not(contains(@class,'ULSxyf'))]")
	private List<WebElement> searchResultList;

/*	@FindBy(xpath = "//div[@id='rso']/div[not(contains(@class,'ULSxyf'))]//h3")
	private List<WebElement> titlesList;

	@FindBy(xpath = "//div[@id='rso']/div[not(contains(@class,'ULSxyf'))]//div[@class='yuRUbf']/a")
	private List<WebElement> urlList;

	@FindBy(xpath= "//div[@id='rso']/div[not(contains(@class,'ULSxyf'))]//div[contains(@class,'VwiC3b yXK7lf MUxGbd')]")
	...
	public String[] extractResultList(String keyword) throws Exception {
		...
		for (WebElement ele : searchResultList) {
			...
			String title = waitForElement(5,ele.findElement(By.xpath(".//h3"))).getText();
			//Extract the url part of the searched item
			String url =  waitForElement(10,ele.findElement(By.xpath(".//div[@class='yuRUbf']/a"))).getAttribute("href");
			String description="";
			//Sometimes we don't get short description for some of the searched item, so we will put it in try catch statement
			try {			
				WebElement e = waitForElement(10, ele.findElement(By.xpath(".//div[contains(@class,'VwiC3b yXK7lf MUxGbd')]")));
			...
			if (ele.getText().toLowerCase().contains(keyword.toLowerCase())) {
				...
				if (title.toLowerCase().contains(keyword)) {
					...
					// check if the description attribute of the search result contains the keyword
			...
			urlArr[index] = url;
			System.out.println("Added to List ith " + index  + " "+ url );
			if (index > 9)
				break;
			index++;
		}
